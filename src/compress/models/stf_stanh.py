...
class SymmetricalTransFormerSoS(CompressionModel):
    def __init__(self,
    ...
    def update(self, scale_table=None,):
...
def modify_bottlenecks(net, factorized_configuration, gaussian_configuration):
    net.entropy_bottleneck = EntropyBottleneckSoS(48*4, 
                                               beta = factorized_configuration["beta"], 
                                                ...
                                                trainable = False,# self.factorized_configuration["trainable"],
                                                device = torch.device("cuda") 
                                                )   

    net.gaussian_conditional = GaussianConditionalSoS(None,
                                                            channels = 320,
                                                            beta = gaussian_configuration["beta"], 
                                                            num_sigmoids = gaussian_configuration["num_sigmoids"], 
                                                            activation = gaussian_configuration["activation"],
                                                            extrema = gaussian_configuration["extrema"], 
                                                            trainable = False,# self.gaussian_configuration["trainable"],
                                                            device = torch.device("cuda")
                                                            )
...
class SymmetricalTransFormerStanh( SymmetricalTransFormer):
    def __init__(self, 
        ...
        self.entropy_bottleneck = EntropyBottleneckSoS(48*4, 
                                               beta = factorized_configuration["beta"], 
                                                num_sigmoids = factorized_configuration["num_sigmoids"], 
                                                activation = factorized_configuration["activation"],
                                                extrema = factorized_configuration["extrema"],
